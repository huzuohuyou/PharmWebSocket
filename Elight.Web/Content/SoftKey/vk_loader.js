...
// 扩展
function loadKeyboard(obj, isChinese, changeLanguage) {

    
    if ($("#softkey").length == 0) {
        $("body").append('<div style="position: absolute; left: 0; top: 0; display: none;"><div id="softkey"></div></div>');
    }

    var containerDiv = $("#softkey").parent();
    containerDiv.show();
    containerDiv.css("z-index", 9100);
    obj = $(obj);
    if (obj.attr("id")) {
        if (obj.offset().left + 870 >= $(window).width()) {
            containerDiv.css("left", $(window).width() - 870);
        }
        else {
            containerDiv.css("left", obj.offset().left);
        }
        if (obj.offset().top + 365 >= $(window).height() + $(window).scrollTop()) {
            containerDiv.css("top", obj.offset().top - 275);
        }
        else {
            containerDiv.css("top", obj.offset().top + 95);
        }
        VirtualKeyboard.changeLanguage = changeLanguage;
